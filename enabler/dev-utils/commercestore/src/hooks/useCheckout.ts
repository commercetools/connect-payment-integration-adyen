...
export function useCheckout(): {
  ...
  const load = useCallback(async (cartId: string, { isDropin = false, onComplete, onError }: LoadOptions = {}) => {
    ...
    try {
      ...
      if (storedEnabled) {
        const stored = await instance.getStoredPaymentMethods({ allowedMethodTypes: [] }).catch(() => []);
        setSavedMethods(stored ?? []);
      }
  ...
  return { cart, enabler, paymentMethods, dropinMethods, savedMethods, loading, error, load, reset };
}
